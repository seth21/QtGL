...
public:
    ...
    std::shared_ptr<T> load(const std::string& path, ResourceConfig config = ResourceConfig()) {
        ...
        bool found = false;
        auto &weakVector = resources[path];
        for (auto &weak : weakVector) {
            res = weak.lock();
            if (res && res->getResourceConfig().isEqual(config)) {
                found = true;
                break;
            }
        }
...
private:
    std::unordered_map<std::string, std::vector<std::weak_ptr<Resource>>> resources;
    ResourceManager(){}
};
